...
<body>
	...
	if ($result->num_rows > 0) {
		while ($row = $result->fetch_assoc()) {
			...
			while ($startDate <= $endDate) {
				...
				$monthlyOrderCounts[$month] += 1; // Assign a value of 1 for each month
	...
	foreach ($monthlyOrderCounts as $month => $count) {
		$orderCounts[] = [
			'y' => $month,
			'a' => $count // No rounding needed since we are assigning 1
		];
	}
								...
								<div class="card card-modern">
									...
									<div class="card-body">
										...
										<div class="row">
											<div class="col">
												<!-- Morris: Area -->
												<div class="chart chart-md chart-bar-stacked-sm my-3" id="revenueChart" style="height: 200px;"></div>
												<script type="text/javascript">
													var revenueChartData = <?php echo json_encode($orderCounts); ?>;
