...
<body>
	...
	// Initialize an array to store monthly order counts
	$monthlyOrderCounts = array_fill_keys(['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'], 0);
	...
	if ($result->num_rows > 0) {
		while ($row = $result->fetch_assoc()) {
			...
			$interval = $startDate->diff($endDate)->m + 1; // Number of months between start and end date

			// Loop through each month between start_date and end_date
			while ($startDate <= $endDate) {
				$month = $startDate->format('M'); // Get the month in short format (e.g., Jan, Feb)
				$monthlyOrderCounts[$month] += 1 / $interval; // Divide the order equally across months
	...
	foreach ($monthlyOrderCounts as $month => $count) {
		$orderCounts[] = [
			'y' => $month,
			'a' => round($count, 2) // Round to 2 decimal places for better readability
		];
	}
													...
													<script type="text/javascript">
														...
														if ($('#revenueChart').get(0)) {
															Morris.Bar({
																...
																hideHover: true,
																grid: false,
																yLabelFormat: function(y) { return Math.round(y); } // Ensure y-axis labels are integers
															});
														}
													</script>
												</div>
